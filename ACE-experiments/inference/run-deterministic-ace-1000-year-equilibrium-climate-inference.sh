...
for model in "${!MODELS[@]}"; do
    ...
    for climate in "${!INITIAL_CONDITION_DATASETS[@]}"; do
        ...
        for ensemble_member in $(seq 0 $((n_members - 1))); do
            ...
            job_name="${DATE}-${model}-${climate}-1000-year-equilibrium-climate-inference-ic${ensemble_member}"
            gantry run \
                ...
                -- bash -c "\
                    ...
                    && \
                    python -I -m fme.ace.inference /tmp/config.yaml --override $override --segments 8 \
                "
        done
    done
done
