...
TEST_EXPERIMENT_DIR="/results/test"
...
for name in "${!MODELS[@]}"; do
    job_name="2026-06-21-test-${name}-split-amip-plus-4K-inference"
    ...
    spin_up_override="\
        ...
        forcing_loader.dataset.data_path=${AMIP_PLUS_4K_DATA_ROOT} \
    ...
    gantry run \
        ...
        -- /bin/bash -c "\
            ...
            && \
            python -I -m fme.ace.inference /tmp/config.yaml --override $spin_up_override \
            && \
            python -I -m fme.ace.inference /tmp/config.yaml --override $train_and_validate_override \
            && \
            python -I -m fme.ace.inference /tmp/config.yaml --override $test_override \
        "
done
